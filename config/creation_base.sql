...
CREATE TABLE `garageparrot`.`users` ( `id` INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY , `name` VARCHAR(100),`surname` VARCHAR(100),`email` VARCHAR(255) NOT NULL UNIQUE,`password` VARCHAR(255)NOT NULL);
-- Création de l’utilisateur BDD
CREATE USER 'user'@'localhost' IDENTIFIED BY '3f7zhhRn4NH69R';
-- création utilisateu administrateur
INSERT INTO garageparrot.users (name, surname, email, password) VALUES ('parrot','vincent','vparrot@mail.com','3f7zhhRn4NH69');
-- Attribution des droits sur la table "users"
GRANT SELECT, INSERT, UPDATE, DELETE ON garageparrot.users TO 'user'@'localhost';
CREATE TABLE `garageparrot`.`roles` ( id INT(11) NOT NULL PRIMARY KEY AUTO_INCREMENT
, name varchar(100) NOT NULL UNIQUE);
GRANT SELECT, INSERT, UPDATE, DELETE ON garageparrot.roles TO 'user'@'localhost';
CREATE TABLE `garageparrot`.`userRoles` ( userID varchar (36) NOT NULL, roleId INTEGER(11) NOT NULL,PRIMARY KEY (userId, roleID),FOREIGN KEY (userId) REFERENCES users(id), FOREIGN KEY (roleId) REFERENCES roles(id));
